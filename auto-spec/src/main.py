...
async def assign_index_contents(client: AsyncOpenSearch, index_name: str):
    ...
    record = lambda: {
        "int_key": random.randint(-(2**31), 2**31 - 1),
        "str_key": fake.text(),
    }
    ...
    return [record() for _ in range(random.randint(1, 100))]
...
async def do_test(client: AsyncOpenSearch):
    ...
    await client.indices.delete(iname, params={"timeout": 120})
...
async def main():
    ...
    try:
        futures = [
            asyncio.create_task(do_test_capturing_result(client)) for _ in range(100)
        ]
        total = 0
